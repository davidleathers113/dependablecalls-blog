...
export function useDropdownState<TValue = unknown>(
  ...
  const close = useCallback(() => {
    ...
    animationTimeoutRef.current = setTimeout(() => {
      // Animation complete - no state to update
    }, animationDuration)
...
/**
 * Higher-order component that provides dropdown state to wrapped component
 */
export function withDropdownState<P extends object>(
  Component: React.ComponentType<P & { dropdown: DropdownStateResult }>
) {
  return function DropdownStateProvider(props: P & { 
    dropdownItems?: DropdownItem[] 
    dropdownOptions?: DropdownStateOptions 
  }) {
    const { dropdownItems = [], dropdownOptions, ...componentProps } = props
    const dropdown = useDropdownState(dropdownItems, dropdownOptions)
    
    return <Component {...(componentProps as P)} dropdown={dropdown} />
  }
}