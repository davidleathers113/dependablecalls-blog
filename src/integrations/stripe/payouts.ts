 ...
 */
const statementDescriptorSchema = z.string()
...
export const updateTransfer = async (
  ...
  try {
    ...
    const transfer = await stripeServerClient.transfers.update(
      transferId,
      allowedUpdates,
      {
...
): Promise<Array<{ supplierId: string; success: boolean; transfer?: Stripe.Transfer; error?: Error }>> => {
  try {
    const now = new Date();
    ...
    const results: Array<{ supplierId: string; success: boolean; transfer?: Stripe.Transfer; error?: Error }> = [];
    ...
    for (let i = 0; i < eligibleSuppliers.length; i += maxConcurrency) {
      ...
      const batchPromises = batch.map(async (supplier) => {
        try {
          ...
          if (error instanceof Error && 'isDuplicate' in error && (error as Error & { isDuplicate: boolean }).isDuplicate) {
