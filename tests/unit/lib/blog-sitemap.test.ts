...
const mockTaxonomyService = vi.mocked(TaxonomyService);
...
describe('BlogSitemapGenerator', () => {
  ...
  describe('Sitemap generation', () => {
    beforeEach(() => {
      ...
      mockTaxonomyService.getCategories.mockResolvedValue(mockCategories);
      mockTaxonomyService.getTags.mockResolvedValue([]);
    });
  ...
  describe('Caching', () => {
    it('should cache sitemap results', async () => {
      ...
      expect(mockPostService.getPosts).toHaveBeenCalledTimes(3); // Initial calls for posts, categories, authors
    });

    it('should clear cache when requested', async () => {
      await generator.generateBlogSitemap();
      const callCountAfterFirst = mockPostService.getPosts.mock.calls.length;
      
      generator.clearCache();
      await generator.generateBlogSitemap();
      const callCountAfterSecond = mockPostService.getPosts.mock.calls.length;
